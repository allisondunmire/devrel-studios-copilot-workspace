...
It works on already-clean files too (multi-line cues are preserved), so it is
safe to run on any .vtt/.srt.
...
def cue_text(text_lines, rolling):
    """Collapse a cue's text lines to its spoken text.

    Raw YouTube-ASR files use a rolling window: each cue restates the previous
    settled line and appends the next one, so the newly spoken text is the LAST
    non-empty line (earlier lines are already captured by the previous cue).
    For those files we keep only the last line to avoid duplication.

    Already-clean files have no rolling window, so genuine multi-line cues are
    preserved by joining all their lines. `rolling` is detected once for the
    whole file (see is_rolling), which keeps this safe on already-clean input.
    """
    cleaned = [clean_text(ln) for ln in text_lines]
    cleaned = [ln for ln in cleaned if ln]
    if not cleaned:
        return ""
    return cleaned[-1] if rolling else "\n".join(cleaned)


def is_rolling(raw):
    """Raw YouTube-ASR captions carry inline word-timing tags like
    <00:00:02.720><c> across</c>. Their presence marks the rolling format."""
    return bool(re.search(r"<\d\d:\d\d:\d\d[.,]\d\d\d>", raw))


def build_clean(cues, rolling):
    """One segment per cue's spoken text; de-dup consecutive rolling repeats."""
    segs = []
    for c in cues:
        txt = cue_text(c["text_lines"], rolling)
        if txt:
...
def main():
    ...
    cues = parse_cues(raw)
    clean = build_clean(cues, is_rolling(raw))
