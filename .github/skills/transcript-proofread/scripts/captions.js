...
const TOKENS_DIR = path.join(process.env.USERPROFILE || process.env.HOME, '.copilot', 'youtube-tokens');
...
// Fetch the current caption tracks for a video (used by verify).
async function listTracks(tok, videoId) {
  const r = await jsonReq('GET', `/youtube/v3/captions?part=snippet&videoId=${videoId}`, tok);
...
// After an insert/update, re-list tracks and warn if YouTube is likely still
// showing the auto-generated (ASR) captions instead of our corrected track.
// Note: the Data API can't report which track a viewer actually sees; this
// checks that a published (non-draft) standard track for the language exists.
//
// evaluateUpload() is the pure, network-free core (deterministic → unit-tested
// in captions.test.mjs); verifyUpload() wraps it with the live list call.
function evaluateUpload(tracks, expectedId, language) {
  const mine = tracks.find(t => t.id === expectedId)
    || tracks.find(t => t.trackKind === 'standard' && (!language || t.language === language));
  const warnings = [];

  if (!mine) {
    warnings.push('Corrected track not found when re-listing captions — the upload may not have taken. Re-run and check `list`.');
  } else {
    if (mine.isDraft) {
      warnings.push('Corrected track uploaded but is still a DRAFT — viewers will NOT see it until it is published (re-run without --draft).');
    }
    const asr = tracks.find(t => t.trackKind === 'ASR' && (!language || t.language === language));
    if (asr && mine.isDraft) {
      warnings.push('YouTube still has an auto-generated (ASR) track for this language and no published standard track — viewers are likely still seeing the auto-captions.');
    } else if (asr) {
      warnings.push('An auto-generated (ASR) track still exists. Your published standard track should take precedence, but it can take a few minutes to propagate — re-check with `list` shortly.');
    }
  }

  return { verified: !!mine && !mine.isDraft, track: mine || null, warnings };
}

async function verifyUpload(tok, videoId, expectedId, language) {
  const tracks = await listTracks(tok, videoId);
  return { ...evaluateUpload(tracks, expectedId, language), tracks };
}
...
function uploadCaption(method, apiPath, tok, metadata, filePath) {
  return new Promise((resolve, reject) => {
    ...
    const captionType = /\.srt$/i.test(filePath) ? 'application/octet-stream' : 'text/vtt';
    const meta = Buffer.from(
      `--${boundary}\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n` +
      JSON.stringify(metadata) + `\r\n`);
    const fileHeader = Buffer.from(`--${boundary}\r\nContent-Type: ${captionType}\r\n\r\n`);
...
async function main() {
  ...
  try {
    if (cmd === 'list') {
      ...
      const v = await verifyUpload(tok, videoId, r.id, lang);
      v.warnings.forEach(w => console.error('⚠️  ' + w));
      console.log(JSON.stringify({ success: true, id: r.id, snippet: r.snippet, verified: v.verified, warnings: v.warnings }, null, 2));
    } else if (cmd === 'update') {
      ...
      const videoId = r.snippet && r.snippet.videoId;
      const v = videoId
        ? await verifyUpload(tok, videoId, r.id, r.snippet && r.snippet.language)
        : { verified: false, warnings: ['Could not verify: update response did not include a videoId. Re-check with `list <videoId>`.'] };
      v.warnings.forEach(w => console.error('⚠️  ' + w));
      console.log(JSON.stringify({ success: true, id: r.id, snippet: r.snippet, verified: v.verified, warnings: v.warnings }, null, 2));
    } else {
